...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleMexNewsletterNotification = (id, node) => {
        ...
        if (operation === Types_1.MexOperations.PROMOTE || operation === Types_1.MexOperations.DEMOTE) {
            let action = operation === Types_1.MexOperations.PROMOTE ? 'promote' : 'demote';
            contentPath = content.data[action === 'promote' ? Types_1.XWAPaths.PROMOTE : Types_1.XWAPaths.DEMOTE];
        ...
        switch (nodeType) {
            case 'privacy_token':
                ...
                for (const { attrs, content } of tokenList) {
                    ev.emit('chats.update', [{ id: attrs.jid, tcToken: content }]);
                }
                break;
            case 'newsletter':
            ...
            case 'w:gp2':
                ...
                break;
            case 'mediaretry':
                ev.emit('messages.media-update', [(0, Utils_1.decodeMediaRetryNode)(node)]);
                break;
            case 'encrypt':
            ...
            case 'picture':
                ...
                if ((0, WABinary_1.isJidGroup)(from)) {
                    ...
                    result.participant = (setPicture || delPicture).attrs.author;
                }
                break;
            case 'account_sync':
                    ...
                    ev.emit('creds.update', {
                        accountSettings: {
                            ...
                            defaultDisappearingMode: {
                                ephemeralExpiration: +child.attrs.duration,
                                ephemeralSettingTimestamp: +child.attrs.t,
                            },
                        }
                    });
                }
    ...
    const willSendMessageAgain = (id, participant) => {
        ...
        return (msgRetryCache.get(key) || 0) < maxMsgRetryCount;
    };

    const updateSendMessageAgainCount = (id, participant) => {
        const key = `${id}:${participant}`;
        msgRetryCache.set(key, (msgRetryCache.get(key) || 0) + 1);
    };
        ...
        try {
            await processingMutex.mutex(async () => {
                ...
                if (attrs.type === 'retry') {
                    ...
                    if (willSendMessageAgain(ids[0], key.participant) && key.fromMe) {
        ...
        try {
            await processingMutex.mutex(async () => {
                ...
                if (msg) {
                    ...
                    await upsertMessage(WAProto_1.proto.WebMessageInfo.fromObject(msg), 'append');
                }
            });
        } finally {
            await sendMessageAck(node);
            ...
            await processingMutex.mutex(async () => {
                ...
                if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                    retryMutex.mutex(async () => {
                        if (ws.isOpen) await sendRetryRequest(node, !encNode);
                    });
                } else {
                    let type = undefined;
                    ...
                    if (category === 'peer') type = 'peer_msg';
                        ...
                        type = 'sender';
                        if ((0, WABinary_1.isJidUser)(msg.key.remoteJid)) participant = author;
    ...
    const requestPlaceholderResend = async (messageKey) => {
        if (placeholderResendCache.get(messageKey === null || messageKey === void 0 ? void 0 : messageKey.id)) return;
        placeholderResendCache.set(messageKey === null || messageKey === void 0 ? void 0 : messageKey.id, true);

        await (0, Utils_1.delay)(5000);
        if (!placeholderResendCache.get(messageKey === null || messageKey === void 0 ? void 0 : messageKey.id)) return 'RESOLVED';
    ...
    const handleCall = async (node) => {
        ...
        const call = {
            ...
            id: callId,
            date: new Date(+node.attrs.t * 1000),
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        if (attrs.error) {
            ev.emit('messages.update', [{ key, update: { status: Types_1.WAMessageStatus.ERROR } }]);
        }
    };
        ...
        const enqueue = (type, node) => {
            ...
            const promise = async () => {
                while (nodes.length && ws.isOpen) {
                    ...
                    await nodeProcessorMap.get(type)(node);
                }
                isProcessing = false;
            };
            promise().catch(error => onUnexpectedError(error, 'offline nodes'));
        };
        return { enqueue };
    };
    ...
    const processNode = (type, node, identifier, exec) => {
        if (!!node.attrs.offline) offlineNodeProcessor.enqueue(type, node);
        else {
            ev.buffer();
    ...
    ws.on('CB:ack,class:message', handleBadAck);
